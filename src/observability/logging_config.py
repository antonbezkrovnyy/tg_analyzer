...
class CustomJsonFormatter(jsonlogger.JsonFormatter):
    ...
    def add_fields(
        ...
        # Add service name
        log_record["service"] = "tg_analyzer"


def setup_logging(
    level: str = "INFO",
    log_format: str = "json",
    service_name: str = "tg_analyzer",
    ...
    if loki_url:
        try:
            ...
            root_logger.info(
                f"Loki handler initialized for service: {service_name}",
                extra={"loki_url": loki_url},
            )
        except ImportError:
            root_logger.warning(
                "logging_loki not installed, skipping Loki handler. "
